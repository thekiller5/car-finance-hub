...
app.get('/client-request.html', (req, res) => { res.sendFile(path.join(__dirname, 'client-request.html')); });
app.get('/login.html', (req, res) => { res.sendFile(path.join(__dirname, 'login.html')); });
app.get('/admin-dashboard.html', (req, res) => { res.sendFile(path.join(__dirname, 'admin-dashboard.html')); });
app.get('/garage-dashboard.html', (req, res) => { res.sendFile(path.join(__dirname, 'garage-dashboard.html')); });
app.get('/dealer-login.html', (req, res) => { res.sendFile(path.join(__dirname, 'dealer-login.html')); });
app.get('/dealer-dashboard.html', (req, res) => { res.sendFile(path.join(__dirname, 'dealer-dashboard.html')); });
...
// מאגר חוות דעת התחלתי המשויך למגרשים (ניתן להוסיף או לעדכן)
let dealerReviews = [
...
let dealersDb = {}; // שמירת מגרשים רשומים
...
app.post('/api/client/request', (req, res) => {
    ...
    clientRequests.push({ 
        ...
        financeProvider, 
        signature, 
        dealerName: dealerName || "מגרש הצפון", // ברירת מחדל אם לא נבחר מגרש
...
// ** חדש: API עבור בעלי מגרשים - החזרת לקוחות וחוות דעת המסוננים אך ורק לפי שם המגרש **
...
// התחברות אדמין ראשית מעודכנת עם הסיסמה החדשה
...
// 1. בקשת קוד אימות לבעל מגרש (8 ספרות בהתאם ל-HTML)
app.post('/api/dealer/request-code', (req, res) => {
    ...
    console.log(`\n========================================`);
    console.log(`[DEALER LOGIN] קוד אימות בן 8 ספרות למספר ${phone}: ${pin}`);
    console.log(`========================================\n`);

    res.json({ success: true, message: 'קוד הופק בטרמינל' });
});

// 2. אימות קוד לבעל מגרש
app.post('/api/dealer/verify-code', (req, res) => {
    ...
    res.status(400).json({ success: false, message: 'קוד האימות שגוי. בדוק את הטרמינל.' });
});

// 3. רישום בעל מגרש חדש
app.post('/api/dealer/register', (req, res) => {
    ...
    console.log(`\n========================================`);
    console.log(`[DEALER REGISTER] מגרש חדש: ${dealerName} | קוד למספר ${phone}: ${pin}`);
    console.log(`========================================\n`);
